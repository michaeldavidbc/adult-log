...
export default function App() {
  ...
  const getRec = () => {
    if (livePercentage <= 30) return { title: 'OPERATIVO', plan: 'basic', color: 'text-[#00FF41]', border: 'border-[#00FF41]', drama: "Has logrado engañar al sistema. Eres un adulto 'funcional', pero cuidado: la estabilidad es un espejismo." };
    if (livePercentage <= 60) return { title: 'RIESGO MODERADO', plan: 'standard', color: 'text-yellow-500', border: 'border-yellow-500', drama: "Tu vida es un equilibrio precario. Estás improvisando, y el público (tus finanzas) lo sabe." };
    return { title: 'COLAPSO ESTRUCTURAL', plan: 'vip', color: 'text-red-500', border: 'border-red-500', drama: "¡Alerta Roja! Tu nivel de fraude es tan alto que ya no eres un adulto, eres un caos organizado esperando una catástrofe." };
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {view === 'TEST' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8 animate-fade-in">
            <div className="border border-zinc-800 p-6 bg-zinc-950">
              <div className="mb-6"><div className="flex justify-between text-xs mb-1"><span>Fraude:</span><span>{livePercentage}%</span></div><div className="h-2 w-full bg-zinc-900 border border-zinc-800"><div className="h-full bg-[#ef4444] transition-all" style={{ width: `${livePercentage}%` }} /></div></div>
        ...
        {view === 'RESULTS_BANNER' && (
          <div className="animate-fade-in text-center mt-12 p-8 border border-zinc-800 bg-zinc-950">
            ...
            <button onClick={() => setView('SERVICES')} className="bg-[#00FF41] text-black px-12 py-4 font-bold uppercase text-lg">Ver Soluciones de Rescate</button>
          </div>
        )}
        ...
        {view === 'SERVICES' && (
          <div className="space-y-12 animate-fade-in mt-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { id: 'basic', title: 'Alquiler de Adulto', price: '$9.99/semana', desc: '¿Pánico por una llamada? Alquilamos un Adulto Responsable por 1 hora. Realiza trámites y firma documentos por ti.' },
                { id: 'standard', title: 'Soporte Logístico', price: '$49.99/semana', desc: 'Auditoría de despensa, automatización de pagos y eliminación de carga mental. Transformamos tu caos en sistema.' },
                { id: 'vip', title: 'Rescate Ejecutivo', price: '$299.99/semana', desc: 'Control total de finanzas, agenda y dignidad. Si esto no te salva, nada lo hará. Incluye plan de contingencia.' }
              ].map((s, i) => (
                <div key={i} className={`border p-6 flex flex-col justify-between ${getRec().plan === s.id ? 'border-[#00FF41] ring-1 ring-[#00FF41] bg-zinc-900' : 'bg-zinc-950 border-zinc-800'}`}>
                  ...
                  <p className="text-xs text-zinc-400 mb-6 italic">{s.desc}</p>
